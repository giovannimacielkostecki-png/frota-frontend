...
export default function Abastecimento() {
  ...
  async function handleSubmit(e) {
    ...
    await criar({ ...form, kmAtual: Number(form.kmAtual), litros: Number(form.litros), valorTotal: Number(form.valorTotal) });
    toast.success('Abastecimento registrado!');
    setForm(p => ({ ...p, kmAtual: '', litros: '', valorTotal: '', posto: '' }));
    refetch();
  }
        ...
        <Card>
          ...
          <form onSubmit={handleSubmit} style={{ padding: 16 }}>
            <FormGrid>
              ...
              <Input label="Litros" type="number" step="0.01" placeholder="320" value={form.litros} onChange={e => set('litros', e.target.value)} required />
              <Input label="Valor total (R$)" type="number" step="0.01" placeholder="1920.00" value={form.valorTotal} onChange={e => set('valorTotal', e.target.value)} required />
          ...
          <div style={{ padding: 16, height: 240 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={(resumo || []).map(r => ({
                ...
                <Bar dataKey="consumo" radius={[5,5,0,0]}
                  fill="#f0a500"
